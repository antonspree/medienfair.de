...
export function StepsSection() {
  return (
    <section id="ablauf" className={`flex flex-col gap-12 overflow-hidden bg-[#FAFAFA] py-16 ${fontInter}`}>
      <Reveal className="mx-auto flex w-full max-w-3xl flex-col items-center gap-[13px] px-5 text-center">
        ...
        <h2 className="max-w-md text-3xl font-medium leading-tight text-[#1b263b] sm:text-4xl sm:leading-[1.15]">
          Get started
        </h2>
      ...
      <div className="mx-auto grid w-full max-w-5xl grid-cols-1 gap-4 px-5 md:grid-cols-3 md:items-stretch">
        {steps.map((step, i) => (
          <Reveal key={step.num} delay={i * 0.1} className="h-full">
            <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-[#1b263b]/[0.08] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
              <div className="border-b border-solid border-[#1b263b]/[0.08]">
                {step.illustration}
              </div>
              <div className="flex flex-1 flex-col items-center justify-center gap-3 p-6 text-center">
                <span className="flex h-9 w-10 items-center justify-center rounded-full bg-[#1b263b]/[0.08] text-lg font-medium text-[#fca311]">
                  {step.num}
                </span>
                <p className="text-lg font-medium leading-6 text-[#1b263b]">{step.text}</p>
              </div>
            </article>
          </Reveal>
        ))}
